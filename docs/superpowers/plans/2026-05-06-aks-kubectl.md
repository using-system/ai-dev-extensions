# AKS kubectl Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a skill that intercepts all kubectl commands and routes them through `az aks command invoke` for private AKS cluster access.

**Architecture:** Single skill file (`skills/aks-kubectl/SKILL.md`) with YAML frontmatter + rules. Cluster identity (resource-group, name) stored in persistent memory on first use.

**Tech Stack:** Azure CLI (`az aks command invoke`), shell scripting

---

## File Structure

- Create: `skills/aks-kubectl/SKILL.md` — the skill definition
- Create: `memory/aks_cluster.md` — persistent memory for cluster identity (created at runtime when user provides values, not part of this plan's deliverables)
...
**Files:**
- Create: `skills/aks-kubectl/SKILL.md`

- [ ] **Step 1: Create the skill directory**

```bash
mkdir -p skills/aks-kubectl
```

- [ ] **Step 2: Write the SKILL.md file**

Create `skills/aks-kubectl/SKILL.md` with the following complete content:
...
# Step 2: Send it via --file
az aks command invoke -g <RG> -n <CLUSTER> \
  -c "bash /tmp/aks-cmd.sh" \
  -f /tmp/aks-cmd.sh
```
...
# Complex command pattern (file)
cat > /tmp/aks-cmd.sh << 'EOF'
<complex command here>
EOF
az aks command invoke -g <RG> -n <CLUSTER> -c "bash /tmp/aks-cmd.sh" -f /tmp/aks-cmd.sh
...
```bash
head -3 skills/aks-kubectl/SKILL.md
```

Expected: YAML frontmatter with `---`, `name: aks-kubectl`, `description: "Use when..."`.

- [ ] **Step 4: Commit**

```bash
git add skills/aks-kubectl/SKILL.md
git commit -m "feat(skills): add aks-kubectl skill for private cluster access"
```

---

### Task 2: Ask the user for cluster identity and store in memory

This task is done **at runtime** (not code to write). When the skill is first invoked:

- [ ] **Step 1: Ask the user for resource-group and cluster name**

- [ ] **Step 2: Create the memory file**

Create `memory/aks_cluster.md`:

```markdown
---
name: AKS cluster identity
description: Resource group and cluster name for the private AKS cluster used with az aks command invoke
type: reference
---

- **resource-group:** `<value from user>`
...
```markdown
- [AKS cluster identity](aks_cluster.md) — resource-group and name for az aks command invoke
```
